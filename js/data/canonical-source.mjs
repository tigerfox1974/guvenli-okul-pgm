const DISTRICT_DICTIONARY = Object.freeze([
  { value: 'Lefkoşa', label: 'Lefkoşa', aliases: ['Lefkosa'] },
  { value: 'Gazimağusa', label: 'Gazimağusa', aliases: ['Gazimağusa', 'Gazimagusa', 'Magusa'] },
  { value: 'Girne', label: 'Girne', aliases: [] },
  { value: 'Güzelyurt', label: 'Güzelyurt', aliases: ['Guzelyurt'] },
  { value: 'İskele', label: 'İskele', aliases: ['Iskele'] },
  { value: 'Lefke', label: 'Lefke', aliases: [] }
]);
...
export const CANONICAL_SCHOOLS = Object.freeze(
  RAW_SCHOOLS.map(school => {
    const district = resolveSchoolDistrict(school);
...
export function getCanonicalSchoolById(schoolId) {
  const numericId = Number(schoolId);
  if (!Number.isInteger(numericId) || numericId <= 0) return null;
  return CANONICAL_SCHOOLS.find(school => Number(school.id) === numericId) || null;
}
...
export function resolveSchoolDistrict(school) {
  const baseDistrict = normalizeDistrictName(school && (school.district || school.region) || '', 'Belirsiz');
  const isLefkeSchool = /\blefke\b/i.test(String(school && school.name || ''));

  if (LEFKE_DISTRICT_POLICY === 'group-under-guzelyurt') {
    if (isLefkeSchool || baseDistrict === 'Lefke') return 'Güzelyurt';
